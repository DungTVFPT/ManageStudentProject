...
public interface ScoreRepository extends JpaRepository<Score, String>{
	...
	//ac
	@Transactional    
	@Query(value = "SELECT semester FROM tbl_score group by semester", nativeQuery = true)
	List<String> getListSemester();
	@Transactional    
	...
	@Transactional 
	@Query(value = "SELECT semester FROM tbl_score WHERE account_id = ?1 and status_register = 0 group by semester", nativeQuery = true)
	List<String> getListSemesterByIdUser(String account_id);
	...
	List<Score> getListAllScoreSemesterByIdUser(String account_id);
	
	@Query(value = "SELECT * FROM tbl_score WHERE id = ?1", nativeQuery = true)
